...
class OmniscientStateEstimator:
    ...
    def update(self,o):
        ...
        sensor reading."""
        global BOUNCE_RATIO, previous_ratio, counter 
        ...
        for idx in range(len(o.names)):
            cur_name = o.names[idx]
            cur_vel = o.velocities[idx]
            cur_h = o.positions[idx][2]
            cur_KE_ratio = 0.5 * sum([v_i**2.0 for v_i in cur_vel])
            for idx in range(len(o.positions)):
                cur_name = o.names[idx]
                cur_h = o.positions[idx][2]
                cur_vel = o.velocities[idx]
                cur_KE_ratio = sum([v ** 2.0 for v in cur_vel])
                if cur_h > 0 and cur_KE_ratio > 0.5:
                    if cur_name not in previous_ratio.keys():
                        cur_deduction = 1
                    else:
                        cur_deduction = cur_KE_ratio/previous_ratio[cur_name]
                        if cur_deduction < 0.5:
                            print(cur_deduction)
                previous_ratio[cur_name] = cur_KE_ratio
